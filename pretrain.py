...
HOW TO RUN:
    # 1. Train the BPE tokenizer once on your corpus:
    ...
    # 3. Multi-GPU:
    torchrun --nproc_per_node=2 pretrain.py --corpus_dir data/raw_text
...
class PretrainConfig:
    ...
    tokenizer_vocab_size: int = 8_000  # only used with --train_tokenizer
    ...
    train_tokenizer: bool = False  # if True, train BPE and exit
...
def _stream_tokens(
    ...
    Documents are concatenated end-to-end with <EOS> between them — no padding.
    input = chunk[:-1],  target = chunk[1:]  (next-token prediction).
    """
    ...
    # leftover (zero-pad to fill final chunk)
    if len(buffer) > 1:
...
def build_dataloaders(
    ...
    if not all_files:
        raise FileNotFoundError(f"No .txt/.jsonl files found in {config.corpus_dir!r}")

    random.seed(config.seed)
    random.shuffle(all_files)

    if len(all_files) == 1:
        train_files = val_files = all_files  # single file: share it
    else:
...
def save_checkpoint(model, optimizer, step, val_loss, config, model_config, path):
    ...
    print(f"  [ckpt] {path}  iter={step}  val_loss={val_loss:.4f}  ppl={ppl:.1f}")
...
def pretrain(config: PretrainConfig):
    ...
    if master:
        ...
        print(
            f"Pre-training  iters={config.max_iters}  eff_batch={eff}  device={device}"
        )
    ...
    for step in range(start_iter, config.max_iters):
        ...
        for micro in range(config.grad_accumulation_steps):
            try:
                ...
                try:
                    ...
                    raise RuntimeError(
                        "Dataloader empty after reset — corpus too small for one batch."
                    )
        ...
        if master and (step + 1) % config.log_interval == 0:
            ...
            print(
                f"{step+1:>8} | {lr:>10.2e} | {avg:>10.4f} | "
                f"{math.exp(min(avg,20)):>8.1f} | {tok_count/dt:>10,.0f}"
            )
        ...
        if master and (step + 1) % config.eval_interval == 0:
            ...
            print(
                f"{'VAL':>8} | {lr:>10.2e} | {val_loss:>10.4f} | "
                f"{math.exp(min(val_loss,20)):>8.1f} |"
            )
            save_checkpoint(
                model,
                optimizer,
...
if __name__ == "__main__":
    ...
    if config.train_tokenizer:
        files = _corpus_files(config.corpus_dir)
        if not files:
            raise FileNotFoundError(f"No files in {config.corpus_dir!r}")
        print(
            f"Training BPE on {len(files)} file(s)  vocab_size={config.tokenizer_vocab_size}..."
        )
        corpus = []
        for fp in files:
            text = Path(fp).read_text(errors="replace")
            ...
            corpus.append(text)
        tok = BPETokenizer()
        tok.train("\n".join(corpus), vocab_size=config.tokenizer_vocab_size)
        os.makedirs(os.path.dirname(config.tokenizer_path) or ".", exist_ok=True)
        tok.save(config.tokenizer_path)
        print(f"Saved to {config.tokenizer_path}. Now run without --train_tokenizer.")
    else:
        pretrain(config)
